"""
 ...
 but difference is this script also remove outlier datas,
...
for row in range(len(dataset)) :
    if abs(dataset.at[row,'PricePerMeter']-price_per_meter_df.at[int(dataset.at[row,'District']),'avarage']) < 100 :
...
print("\n================================\nDeleting outlier datas and replacing avarage in PricePerMeter column finished")
dataset = dataset.iloc[: , 1:]
print(dataset)
dataset.to_csv(DATASETV3_50_ADDRESS)